...
export default async function ProfilePage() {
  ...
  // Gracefully handle DB not ready yet
  let profile: Record<string, unknown> | null = null
  let submitted: ProfileItem[] = []
  let voted: VoteItem[] = []
  ...
  try {
    ...
    submitted = (submittedRes.data as unknown as ProfileItem[]) ?? []
    voted = (votedRes.data as unknown as VoteItem[]) ?? []
  } catch {
    // DB schema not applied yet — show skeleton profile
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-5">
        ...
        <div>
          ...
          <span className="inline-flex mt-1 items-center text-xs font-medium px-2 py-0.5 rounded-full bg-primary/10 text-primary capitalize">
            {(profile?.role as string | null) ?? 'user'}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {submitted.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            {profile ? "You haven't submitted any items yet." : 'No data available yet.'}
          </p>
        ) : (
      ...
      <div className="space-y-3">
        ...
        {voted.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            {profile ? "You haven't voted on any items yet." : 'No data available yet.'}
          </p>
        ) : (
          <div className="space-y-2">
            {voted
